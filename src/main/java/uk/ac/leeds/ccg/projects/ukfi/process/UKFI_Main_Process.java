 ...
 * the License.
 */
package uk.ac.leeds.ccg.projects.ukfi.process;
...
import uk.ac.leeds.ccg.projects.ukfi.core.UKFI_Environment;
import uk.ac.leeds.ccg.projects.ukfi.core.UKFI_Object;
...
import uk.ac.leeds.ccg.projects.ukfi.chart.WIGB_LineGraph;
import uk.ac.leeds.ccg.projects.ukfi.core.UKFI_Strings;
...
//    HashMap<Short, Byte>[] GORLookups;
    ...
    public static void main(String[] args) {
        try {
            ...
            UKFI_Environment env = new UKFI_Environment(de);
    ...
    public void run() throws IOException, ClassNotFoundException {
         ...
         * Init subset.
         */
        subset0 = (HashSet<WaAS_W1ID>) hh.getSubset(4);
...
//        MNumbNW5 (Number of mortgages)
     ...
     */
    protected void processDataForAnastasia(boolean addExtraCASEIDs) throws IOException, ClassNotFoundException {
        String outfilename = "Anastasia";
        if (addExtraCASEIDs) {
            outfilename += "WithExtraCASEIDs";
        }
        outfilename += ".csv";
        Path dataForAnastasia = Paths.get(files.getOutputDir().toString(), outfilename);
        /**
         * Write out header.
         */
        try (PrintWriter pw = Generic_IO.getPrintWriter(dataForAnastasia, false)) {
             ...
             * Write out values.
             */
            HashMap<WaAS_CollectionID, WaAS_Collection> cs = env.we.data.dataSimple;
            ...
            while (ite.hasNext()) {
                ...
                env.env.log("Collection ID " + cID);
                WaAS_Collection c = env.we.data.getCollectionSimple(cID);
                HashMap<? super Data_RecordID, ? super Data_Record> cr = c.data;
                Iterator<? super Data_RecordID> ite2 = cr.keySet().iterator();
                while (ite2.hasNext()) {
...
//                        }
     ...
     * @param breaks
     */
    protected void initialiseSubsets(TreeMap<Integer, Double> breaks) throws FileNotFoundException, IOException, ClassNotFoundException {
        /**
        ...
        while (ited.hasNext()) {
            HPROPW0 = ited.next();
            if (HPROPW0 == 0) {
                env.env.log("Adding break[" + breakIndex + "] " + HPROPW1);
                breaks.put(breakIndex, HPROPW1);
                breakIndex++;
                env.env.log("Adding break[" + breakIndex + "] " + HPROPW0);
        ...
        while (ited.hasNext()) {
            ...
            //env.env.log("HPROPW " + HPROPW + " count " + count);
            divc += count;
            if (divc > ndiv8) {
                env.env.log("HPROPW (" + HPROPW0 + ", " + HPROPW1 + ") count "
                        + divc0);
                env.env.log("Adding break[" + breakIndex + "] " + HPROPW1);
                breaks.put(breakIndex, HPROPW1);
                breakIndex++;
    ...
    protected void sumariseAndGraphDataForSubsets(BigDecimal yIncrement,
            TreeMap<Integer, Double> breaks) throws FileNotFoundException, IOException, IOException, ClassNotFoundException {
        for (int decile = 1; decile < 10; decile++) {
            subset = subsets[decile - 1];
            String name;
            if (decile < 9) {
            ...
            while (ite.hasNext()) {
                ...
                for (byte w = 1; w <= env.we.NWAVES; w++) {
                    switch (w) {
                        case 1:
                            Set<WaAS_W1ID> GORSubsetW1 = GORSubsetsAndLookups.gor_To_w1.get(gor);
                            totals[w] += GORSubsetW1.size();
                            break;
                        case 2:
                            Set<WaAS_W2ID> GORSubsetW2 = GORSubsetsAndLookups.gor_To_w2.get(gor);
                            totals[w] += GORSubsetW2.size();
                            break;
                        case 3:
                            Set<WaAS_W3ID> GORSubsetW3 = GORSubsetsAndLookups.gor_To_w3.get(gor);
                            totals[w] += GORSubsetW3.size();
                            break;
                        case 4:
                            Set<WaAS_W4ID> GORSubsetW4 = GORSubsetsAndLookups.gor_To_w4.get(gor);
                            totals[w] += GORSubsetW4.size();
                            break;
                        default:
                            Set<WaAS_W5ID> GORSubsetW5 = GORSubsetsAndLookups.gor_to_w5.get(gor);
...
//	Value = -6.0	Label = Error Partial
        ...
        if (wave == we.W1) {
            tDVLUKVAL = env.we.data.getData().keySet().stream().mapToLong(x -> {
                ...
                long cDVLUKVAL = c.data.keySet().stream().mapToLong(y -> {
...
//                        w4Records = cr.w4Recs;
            ...
            tDVLUKVAL = env.we.data.getData().keySet().stream().mapToLong(x -> {
                ...
                long cDVLUKVAL = c.data.keySet().stream().mapToLong(y -> {
                    ...
                    if (subset.contains(w1ID)) {
                        WaAS_CombinedRecord cr = (WaAS_CombinedRecord) c.data.get(w1ID);
//                        Map<WaAS_W2ID, Map<WaAS_W3ID, Map<WaAS_W4ID, Map<WaAS_W5ID, WaAS_W5Record>>>> w5Records;
//                        w5Records = cr.w5Recs;
    ...
    public long getFINCVB(HashSet<WaAS_W1ID> subset, byte wave) {
        ...
        if (wave == we.W1) {
            tFINCVB = env.we.data.getData().keySet().stream().mapToLong(x -> {
                WaAS_CollectionID cID = (WaAS_CollectionID) x;
                WaAS_Collection c = env.we.data.getCollection(cID);
                long cFINCVB = c.data.keySet().stream().mapToLong(y -> {
                    WaAS_W1ID w1ID = (WaAS_W1ID) y;
                    byte FINCVB = 0;
                    if (subset.contains(w1ID)) {
                        WaAS_CombinedRecord cr = (WaAS_CombinedRecord) c.data.get(w1ID);
...
//                        w3Records = cr.w3Recs;
            ...
            tFINCVB = env.we.data.getData().keySet().stream().mapToLong(x -> {
                ...
                long cFINCVB = c.data.keySet().stream().mapToLong(y -> {
                    ...
                    if (subset.contains(w1ID)) {
                        WaAS_CombinedRecord cr = (WaAS_CombinedRecord) c.data.get(w1ID);
//                        Map<WaAS_W2ID, Map<WaAS_W3ID, Map<WaAS_W4ID, WaAS_W4Record>>> w4Records;
//                        w4Records = cr.w4Recs;
                        Iterator<WaAS_W2ID> ite = cr.w4Recs.keySet().iterator();
                        while (ite.hasNext()) {
                            WaAS_W2ID w2ID = ite.next();
                            Map<WaAS_W3ID, Map<WaAS_W4ID, WaAS_W4Record>> w4_2 = cr.w4Recs.get(w2ID);
                            Iterator<WaAS_W3ID> ite2 = w4_2.keySet().iterator();
                            while (ite2.hasNext()) {
                                WaAS_W3ID w3ID = ite2.next();
                                Map<WaAS_W4ID, WaAS_W4Record> w4_3 = w4_2.get(w3ID);
...
//                        w5Records = cr.w5Recs;
                        ...
                        while (ite.hasNext()) {
                            WaAS_W2ID w2ID = ite.next();
                            Map<WaAS_W3ID, Map<WaAS_W4ID, Map<WaAS_W5ID, WaAS_W5Record>>> w5_2 = cr.w5Recs.get(w2ID);
                            Iterator<WaAS_W3ID> ite2 = w5_2.keySet().iterator();
                            while (ite2.hasNext()) {
                                WaAS_W3ID w3ID = ite2.next();
                                Map<WaAS_W4ID, Map<WaAS_W5ID, WaAS_W5Record>> w5_3 = w5_2.get(w3ID);
    ...
    public void createLineGraph(String title, String xAxisLabel,
        ...
        WIGB_LineGraph chart = new WIGB_LineGraph(env.env, es, file, format, title,
